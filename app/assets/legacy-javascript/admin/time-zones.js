...
class TimeZones {
  constructor() {
    ...
    this.map = document.getElementById('timezone-map')
    if (!this.map) {
      return
    }
  ...
  loadData(callback) {
    d3.json('https://cdn.jsdelivr.net/npm/world-atlas@2/land-110m.json').then(topology => {
      ...
      d3.json('https://gist.githubusercontent.com/mbostock/f0ae25cf1f057d443ca903277e3eb330/raw/254996390d4e19ef5eb1429103e0138ad08e19d0/timezones.json').then(timezones => {
...
$(document).on('turbolinks:load', () => { new TimeZones() }) // eslint-disable-line no-new
