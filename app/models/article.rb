...
class Article < ApplicationRecord
  ...
  scope :recent, -> { published.where('article_translations.published_at >= ?', 2.weeks.ago) }
  scope :not_recent, -> { published.where('article_translations.published_at < ?', 2.weeks.ago) }
